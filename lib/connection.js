 ...
 * @param {String} [options.organizationId] - Salesforce organizationId used for communityLogin
...
/**
 * Login to a specific community by SOAP web service API.
 *
 * @param {String} username - Salesforce username
 * @param {String} password - Salesforce password (and security token, if required)
 * @param {Callback.<UserInfo>} [callback] - Callback function
 * @returns {Promise.<UserInfo>}
 */
Connection.prototype.communityLoginBySoap = function(username, password, callback) {
  var self = this;
  var logger = this._logger;
  var body = [
    '<se:Envelope xmlns:se="http://schemas.xmlsoap.org/soap/envelope/">',
      '<se:Header><LoginScopeHeader xmlns="urn:partner.soap.sforce.com"><organizationId>' + esc(this.organizationId) + '</organizationId></LoginScopeHeader></se:Header>',
      '<se:Body>',
        '<login xmlns="urn:partner.soap.sforce.com">',
          '<username>' + esc(username) + '</username>',
          '<password>' + esc(password) + '</password>',
